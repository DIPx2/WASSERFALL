...
def get_parse_args():
    ...
    parser.add_argument("--cmd", required=True, help="Имя SQL-команды из таблицы commands")
    ...
    parser.add_argument("--db", action="append", help="Список конкретных баз данных")
    parser.add_argument("--db-exclude", action="append", help="Список баз, которые нужно исключить")
    parser.add_argument("--allow-new-hosts", action="store_true", help="Разрешить добавление новых хостов в known_hosts")
    parser.add_argument("--verbose", action="store_true", help="Показать подробный вывод (JSON‑данные по каждой БД)")
